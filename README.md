...
```bash
# Create verity device from your image
make setup_verity IMAGE=<your-image>
```
...
make run_sev_snp_verity MEMORY=<mem-in-mb>
```

### Step 3: Verify guest integrity

Now that the guest has booted, we can connect to it via SSH and get an
attestation report to verify its integrity.

TODO: this part is still missing. Consider extending our tools or use
`snpguest`/`sev-snp-measure`.
